...
local function find_one_player_by_userid(value, command)
    ...
    if target == nil then command:reply("没有找到 userid 为 " .. user_id .. " 的玩家。") end
    return target
end
...
local function find_one_player_by_slot(value, command)
    ...
    if target == nil then command:reply("没有找到 slot 为 " .. slot .. " 的玩家。") end
    return target
end
...
local function create_request(player, target, mode, command)
    ...
    if mode == "here" then
        command:reply("已邀请 " .. target.name .. " 传送到你身边，" .. request_timeout .. " 秒后过期。")
        target:print_chat(player.name .. " 邀请你传送到其身边。输入 !tpaccept 接受，或 !tpdeny 拒绝。")
    else
...
plugin:command("css_tpalist", {
    ...
    for _, candidate in ipairs(players) do
        ...
        command:reply("[userid: " .. candidate.user_id .. " | slot: " .. candidate.slot .. "] " .. candidate.name .. suffix)
    end
end)
...
plugin:command("css_tpaccept", {
    ...
    if anchor.position == nil then
        command:reply("目标玩家的位置无效，暂时无法传送。")
        return
    end
    ...
    remove_request(request)
    if request.mode == "here" then
        command:reply("已接受 " .. sender.name .. " 的邀请，已传送到其身边。")
        sender:print_chat(target.name .. " 已接受邀请并传送到你身边。")
    else
...
plugin:command("css_tpdeny", {
    ...
    remove_request(request)
    if request.mode == "here" then
        command:reply("已拒绝 " .. request.sender_name .. " 的传送邀请。")
...
plugin:command("css_tpcancel", {
    ...
    remove_request(request)
    if request.mode == "here" then
        command:reply("已取消对 " .. request.target_name .. " 的传送邀请。")
