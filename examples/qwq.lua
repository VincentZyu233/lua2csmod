local plugin = cs.plugin({
    name = "QwQ",
    version = "0.1.1",
    description = "玩家发送 qwq 时回复，并播报真人玩家进服和离服"
})
...
plugin:on("player_chat", function(event)
    ...
    if event.player ~= nil and event.text:lower() == "qwq" then
        event.player:print_chat(cs.colors.green .. "qwq_from_lua!" .. cs.colors.default)
    end
...
plugin:on("player_connect_full", function(event)
    ...
    if event.player ~= nil and not event.player.is_bot and not event.player.is_hltv then
        cs.server.print_chat_all(cs.colors.green .. "qwq_from_lua!!!" .. cs.colors.default)
    end
