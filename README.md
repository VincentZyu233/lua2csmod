...
`examples` 中包含 40 个可独立加载的中文模板和 1 个公共模块示例，覆盖基础命令、原生命令监听、聊天与冷却、事件统计、回合玩法、管理员工具、玩家间传送请求、帧与 Tick 调度、菜单、玩家和武器、无限丢枪、武器复制、弹药、计分板、语音、ConVar、实体、导航、模型、持久化、HUD、地图及模块化脚本。模板包含关键流程、快照时效、身份校验和风险点的中文注释。
